...
The DOAB data provides links to the open monograph itself. The monograph content content is thus potentially available for harvesting, analysis, etc -- although some web navigation may be required. (The code below provides an example, which was used to produce the figure above.)
...
-   [OpenBookPublishers](https://www.openbookpublishers.com/section/92/1) provides title-level usage data on the titles it publishes. [^2]
...
License, Language, Publisher, Gender
## Gender
```{r, doab_gender}
ibrary(gender)
library(genderdata)
...
Year of Pub, Year of Release, Gender Ratio
```{r, timetrend}
library(plotly)
library(ggthemes)
tmp_plot <- doab_df %>%
