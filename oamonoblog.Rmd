...
```{r include=FALSE}
# Find oldest book
# TODO: automatically extract first page image from PDF

oldbook.doi <- DOABmeta.df %>% arrange(`Year of publication`) %>% head(n=1L) 

```

![Die Mitschuldigen -- One of the Oldest OA Monographs](DieMitschuldigen.png){width="430"}
...
[^1]: Since this blog takes the form of a fully replicable analysis, new versions will be released as the data sources it relies on are updated.

# Accessible Data on Open Monographs

The most complete index of open access monographs is the [Directory of Open Access Books](https://www.doabooks.org/) (DOAB), which lists tens of thousands of individual monographs (also known as 'titles'). DOAB makes its metadata index available as open data. In the table below you can browse or edit this.

```{r, doab_run, echo=FALSE, warning=FALSE}
datatable(head(DOABmeta.df,n=1000L), class = 'cell-border stripe',caption="Directory of Open Access Books - Catalog Example",options=list(pageLength=5), extensions="Responsive")
...
TMP.plot <- DOABmeta.df %>% group_by(`Year of publication`) %>% summarize(total=n()) %>% ggplot() + aes(x=`Year of publication`,y=`total`) + geom_bar(stat="identity") + geom_smooth() +
...
This is a reproducible document. The source, along with necessary data is available here: <https://github.com/MIT-Informatics/monograph> . The code relies primarily on the [R]{.ul} language, the [Plot.ly]{.ul} graphics package, and open R libraries (especially [tidyverse]{.ul}, [i2dash]{.ul}, [htmlwidgets]{.ul}, and [crosstalk]{.ul} and Baker's [R Makefiles](https://www.jstatsoft.org/article/view/v094c01)). All references in this document are managed in [Zotero,]{.ul} The most straightforward way to examine and modify the source is to clone the module using [git]{.ul} and then load the project using [Rstudio]{.ul}.
