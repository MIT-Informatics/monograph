...
```
## Idea: Revenue vs. OA
## Question: What revenue are publishers receiving each year?
## Observation: 
```{r  }

# Finding revenue for each publisher
...
publisher_revenue <- revenue_df %>%
  ...
  geom_col() +
  labs(title = "Revenue for Publishers", x = "Top 25% of Publishers", y = "Revenue", color = 'Types of OA') +
  theme(axis.text = element_text(size = rel(0.75))) +
  scale_x_discrete(labels = function(x) str_wrap(str_replace_all(x, "foo", " "), width = 17)) +
